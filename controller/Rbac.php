...
class Rbac
{
    ...
    public function __construct($request)
    {
        ...
        $this->user_type       = isset($this->param['user_type'])?intval($this->param['user_type']):'';
    ...
    {
        $result     = Menu::where('')->order(["sort" => "asc",'id'=>'asc'])->column('*','id');
        $tree       = new Tree();
        $tree->nbsp = '&nbsp;&nbsp;&nbsp;';
        ...
        $str = "<tr id='node-\$id' \$parent_id_node>
                    <td style='padding-left:20px;'>
                        <input name='listorders[\$id]' type='text' size='3' value='\$sort' data='\$id' class='listOrder'>
                    </td>
                    <td>\$id</td>
                    <td>\$spacer  \$name</td>
                    <td>\$app</td>
    ...
    public function authorize()
    {
        ...
        $menu       = Menu::where('')->order(["sort" => "asc",'id'=>'asc'])->column('*','id');
        ...
        $info = self::authorizeHtml($menu,0,'admin_url');
    ...
    public function adminAuthorize()
    {
        ...
        if($this->request->isPost()){//表单处理
            ...
            AuthAccess::where(["role_id" => $this->id,'type'=>'admin','role_type'=>$this->user_type])->delete();
        ...
        //管理员所有角色权限
        $roleId     = AuthRoleUser::innerAuthRole($this->id,$this->user_type);
        if(in_array(1,$roleId)){
            ...
            $AuthAccess   = AuthAccess::where(["role_id"=>["in",$roleId],'role_type'=>0])->column('*','menu_id');

        }


        $info = self::authorizeHtml($menu,$this->user_type,'admin',$AuthAccess);
    ...
    {
        $text       = '';
        $file       = strtr($this->param['file'], '_', DIRECTORY_SEPARATOR);
    ...
    protected function authorizeHtml($menu,$user_type,$type,$authMenu=[])
    {
        if($type == 'admin_url')
            $priv_data  = AuthAccess::where(['role_id'=>$this->id,'type'=>$type])->field("rule_name")->column('menu_id');
        else
            $priv_data  = AuthAccess::where(['role_id'=>$this->id,'role_type'=>$user_type,'type'=>$type])->field("rule_name")->column('menu_id');
        $tree       = new Tree();
        foreach ($menu as $n => $t) {
